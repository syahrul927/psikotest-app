...
interface KraepelinInvitationConfirmationProps {
  slug: string;
  name: string;
}
export const KraepelinInvitationConfirmation = ({
  slug,
  name,
}: KraepelinInvitationConfirmationProps) => {
  ...
  const onSuccessConfirmed = (
    ...
    if (data.step === 0) {
      setAccess(slug, true);
      return router.push(PAGE_URLS.KRAEPELIN_TEST_PROFILE(slug));
    }
    if (data.step === 1) {
      setAccess(data.resultId, true);
      return router.push(PAGE_URLS.KRAEPELIN_TEST(slug));
    }
  };
  ...
  const { mutate, isPending: isLoading } = useConfirmationKraepelinInvitation(
    onSuccessConfirmed,
    onErrorConfirmed,
  );
                ...
                <div className="flex flex-col space-y-8">
                  ...
                  <div className="flex flex-col items-center">
                    ...
                    <p className="text-muted-foreground text-xs text-wrap">
                      Belum tau cara kerja kraepelin? Klik
                      <Link
