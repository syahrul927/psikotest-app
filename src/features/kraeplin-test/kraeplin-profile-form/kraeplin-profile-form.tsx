...
import {
  useStartInvitation,
  type StartKraeplinInvitationResponseType,
} from "@/hooks/api/kraeplin-invitation/use-start-invitation";
import { useUpdateProfileKraeplinInvitation } from "@/hooks/api/kraeplin-invitation/use-update-profile-kraeplin-invitation";
...
export const KraeplinProfileForm = ({ id }: { id: string }) => {
  ...
  const onSuccessStartInvitation = (
    resp: StartKraeplinInvitationResponseType,
  ) => {
  ...
  const { mutateAsync: startTest } = useStartInvitation(
    ...
    () => {
      resetAccess();
      router.push(PAGE_URLS.KRAEPLIN_TEST_CONFIRMATION(id));
    },
  );
  const { mutate, isPending: loading } = useUpdateProfileKraeplinInvitation(
    onSuccessUpdateProfile,
  );

  const onConfirm = async () => {
    await startTest({ id });
    router.push(PAGE_URLS.KRAEPLIN_TEST(id));
  };
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="flex w-full flex-col space-y-3">
                <Link href={PAGE_URLS.KRAEPLIN_TRAINING}>
                  <Button
                    className="w-full"
                    isLoading={loading}
                    type="button"
