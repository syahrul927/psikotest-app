...
import { useAccessInvitation } from "@/hooks/use-access-invitation-kraeplin";
import { useGetLatestKraeplin } from "@/hooks/api/kraeplin-test/use-get-latest-kraeplin";
import { useGetTemplateKraeplin } from "@/hooks/api/kraeplin-test/use-get-template-kraeplin";
import { useSubmitAnswerKraeplin } from "@/hooks/api/kraeplin-test/use-submit-answer-kraeplin";
import { useDisplay } from "@/hooks/use-display";
import { useRouter } from "next/navigation";
import { PAGE_URLS } from "@/lib/page-url";
import { toast } from "sonner";

export const useKraeplinTestLogic = (slug: string) => {
  const router = useRouter();
  const { access, id } = useAccessInvitation();
  const { refetch: fetchLatest, data: latest } = useGetLatestKraeplin(slug, id);
  const { data: templateData, refetch: fetchTemplate } =
    useGetTemplateKraeplin();
  ...
  const { mutate } = useSubmitAnswerKraeplin();
  ...
  useEffect(() => {
    if (access) {
      ...
      router.push(PAGE_URLS.KRAEPLIN_TEST_CONFIRMATION(slug));
    }
  }, [access]);
  ...
  useEffect(() => {
    if (count === 0) {
      if (indexColumn < totalColumn) {
        ...
        router.push(PAGE_URLS.KRAEPLIN_THANKS);
      }
    }
  }, [count]);
